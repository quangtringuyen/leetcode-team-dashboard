...
export function useStreaks() {
    return useQuery<StreakData[]>({
        queryKey: ['streaks'],
        queryFn: async () => {
            const response = await api.get('/api/analytics/streaks');
...
export function useStreakLeaderboard(limit: number = 10) {
    return useQuery<StreakData[]>({
        ...
        queryFn: async () => {
            const response = await api.get(`/api/analytics/streaks/leaderboard?limit=${limit}`);
            return response.data;
        },
        staleTime: 5 * 60 * 1000,
    });
...
export function useStreaksAtRisk() {
    return useQuery<StreakData[]>({
        ...
        queryFn: async () => {
            const response = await api.get('/api/analytics/streaks/at-risk');
            return response.data;
        },
        staleTime: 5 * 60 * 1000,
    });
